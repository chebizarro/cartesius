define(function (require) {
	...
	ViewTemplate.prototype.createView = function(parentElement, viewText, nls, styleText) {
		...
		var me = this;
			Stapling.load('datasource.json', viewText, function (xml, view) {
				view.viewId = _.uniqueId(['bpjscontainer_']);
				view.jQueryElement = $("<span id='" + view.viewId + "'/>");
				var jdom = view.jQueryElement.get(0).appendChild(this);
				//view.jQueryElement.appendTo(this);
				Helpers.Styler.attachScopedCss(view.jQueryElement, styleText);

				//if parent is specified, lets attach the element to parent
				if (parentElement) {
					parentElement.append(view.jQueryElement);
				}
